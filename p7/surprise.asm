;Name: Louisa Houser
;Date: 10/31/23
;Assignment: P7 - Spooky Halloween Surprise
;Purpose: Given a large data set of 24x80, take a pair of numbers, ignore one, and decrypt the other. With the number we want to decrypt we need to subtract the column number from it and then print it, as well as print a new line after each row.
...
colNum: db 0 					; holds column number and will be set to 0 with every new row
num: db 0					; holds number to be printed
...
mov ecx, ROWS 					; move 24 into ecx counter
xor bl, bl    					; set al to 0 
xor esi, esi  					; set eax to 0
...
rows:
	push ecx				; push ecx value onto stack to retrieve later
	mov ecx, COLS				; move 80 into ecx 
	mov BYTE[colNum], 0			; set colNum to 0 
	cols:
		mov bl, [surprise + eax]	; bl holds surprise value to be decrypted
		add eax, 2			; add 2 to eax because we want to skip over the attrib. char
		sub bl, [colNum]		; subtract column number from value we are decrypting
		mov BYTE[num], bl		; move bl decrypted value to num variable
		call print			; call print function
		inc BYTE[colNum]		; increment colNum
		loop cols			; decrement ecx and if != 0 then jmp back to cols
	call newline				; calls newline function
	pop ecx					; pop top off of stack and place into ecx
	loop rows				; decrement ecx and if != 0 then jmp back to rows

jmp lastBreak					; skip over print and newline functions

;------------------------------
;	 PRINT FUNCTION	      
;------------------------------
print:	
	pusha					; save all current registers to stack
	mov eax, 4				;
	mov ebx, 1				;
	mov ecx, num				;	PRINT DECRYPTED CHARACTER SEQUENCE
	mov edx, 1				;
	int 80h					;
	popa					; retrieve registers from stack
	ret					; return to place in nested loop

;--------------------------------
;	 NEWLINE FUNCTION	
;--------------------------------
newline:
	pusha					; save all current regs to stack
	mov eax, 4				;
	mov ebx, 1				;
	mov ecx, cr				;	PRINT NEWLINE SEQUENCE
	mov edx, 1				;
	int 80h					;
	popa					; retrieve regs from stack
	ret					; return to place in nested loop
